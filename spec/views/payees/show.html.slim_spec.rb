...
RSpec.describe 'payees/show', type: :view do
  before do
    ...
    assign(:schedules, payee.schedules.rezort(params[:sort], 'start_date ASC').decorate)
  ...
  it 'renders decorated payee details' do
    ...
    assert_select '.card-body' do
      ...
      assert_select '.row:nth-of-type(3) .form-control-plaintext' do
        assert_select "a[href='#{payee.website}']", text: payee.website
      end
    ...
    assert_select 'table.table' do
      payee.schedules.each_with_index do |schedule, i|
        assert_select "tr:nth-of-type(#{i+1})" do
          ...
          assert_select 'td:nth-of-type(4)', text: (schedule.autopay ? 'Enabled' : 'Disabled')
          assert_select 'td:nth-of-type(5)', text: number_to_currency(schedule.minimum_payment)
          assert_select 'td:nth-of-type(6)' do
            assert_select "a.btn.btn-primary[href='/schedules/#{schedule.id}/edit']", text: 'Edit'
            assert_select "a.btn.btn-danger[href='/schedules/#{schedule.id}']", text: 'Delete'
          end
        end
      end
    end
